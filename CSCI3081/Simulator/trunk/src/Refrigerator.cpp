...
Refrigerator::Refrigerator() {
	...
	if (rand < 0.5) {
		...
		running_ = false;
		ticks_ = (int) boost::uniform_real<double>(onTime_ + 1, onTime_
				/ dutyCycleNotCooking_)(generator);
	}
	std::string name = "DefaultRefrigerator";
	setName(name);
	LoggerPtr refrigeratorLog(Logger::getLogger(getName()));
	LOG4CXX_DEBUG(refrigeratorLog, getName()
			<< " :: Refrigerator instance created.");
}
...
void Refrigerator::activate(std::string args) {
	if (args == "on") {
		cooking_ = true;
	} else if (args == "off") {
		cooking_ = false;
