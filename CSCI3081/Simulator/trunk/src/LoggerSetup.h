...
public:
	bool setUpWorld() {
		...
		PropertyConfigurator::configure(File("test.config"));
		LOG4CXX_INFO(log, "Simulator testing started. Log file opened.");
		return true;
	}

	bool tearDownWorld() {
		LoggerPtr log(Logger::getLogger("Setup"));
		LOG4CXX_INFO(log, "Simulator testing exited. Log file closed.");
		return true;
	}
};
